...
--- Find the location of super implementations of a method or class within the project. 
--- This makes use of the same handler function that is configured to be used as the 'textDocument/implementation'
--- method.
---
...
function M.super_implementation(options)
  ...
  -- Since we are borrowing the handler for the 'textDocument/implementation' method, we should still support the same
  -- options that are available to that handler
  local req_handler
  if options then
    req_handler = function(err, result, ctx, config)
      local client = vim.lsp.get_client_by_id(ctx.client_id)
      local handler = client.handlers[implementationMethodimplementationMethodName] or vim.lsp.handlers[implementationMethodName]
